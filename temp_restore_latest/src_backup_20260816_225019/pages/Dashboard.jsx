...
export default function Dashboard({ athlete, apiFetch }) {
  ...
  const loadChallengeActivities = async (clubId, participants) => {
    ...
    try {
      ...
      const now = new Date();
      const processed = processChallengeData(allActivities, participants, now.getFullYear(), now.getMonth() + 1);
      ...
      {viewMode === 'challenge' ? (
        <div className="challenge-view">
          {loadingChallenge ? (
            <div className="loading">
              <div className="loading__spinner"></div>
              <div className="loading__text">Đang tải dữ liệu Challenge...</div>
            </div>
          ) : (
            <ChallengeTable 
